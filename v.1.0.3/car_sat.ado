...
pr car_sat, eclass
	...
	*********
	if "`table'" != "" {
		reg `Y' i.`strata' i.`A'#i.`strata' `covars' if `touse', robust
	}
	...
	*Check for perfect collinearity
	...
	noi mata: myregress("`Y'", "`Scal'", "`interaction'", "`touse'", "`b'", "`V'", "`N'", "`df'", "`V_H'", "`V_hc'", "`covars'")
...
mata:
	...
	void myregress(string scalar depvar_s, string scalar strata_s, string scalar interact_s, ///
		...
		string scalar n_s, string scalar df_s, string scalar v_h, string scalar v_y, string scalar cov)
	{
		real vector y, Xpy, beta, e2, N_s, T, I_S
		real matrix X, XpXi, XpeepX, B, V_H, R, vc, S_cal, V_hc
